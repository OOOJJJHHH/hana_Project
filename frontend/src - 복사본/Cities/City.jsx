...
import axios from "axios"; // useNavigate 훅 임포트
...
function City(){
    ...
    const handleClick = async (Cindex) => {
        updateCity(Cindex);
        // PUT 요청을 보내는 함수
            ...
            {contents.map((content, index) => (
                <div key={index} style={cityStyle_default}>
                    <div style={city_content}>
                        <strong>{content.title}sdf</strong>
                        <p style={{ fontSize: '15px' }}>{content.content}</p>
                    </div>
                    <img src={content.imag} style={backImg} />
